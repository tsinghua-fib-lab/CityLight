...
class SharedReplayBuffer(object):
    def __init__(self, args, num_agents, obs_space, share_obs_space, act_space, max_neighbor_num=None, parallel_num=None, neighbor_idxs=None, phase_relation=None, neighbor_type=None, neighbor_distances=None, neighbor_masks=None):    
        ...
        if 'Dict' in obs_shape.__class__.__name__:
            ...
            if args.agg == 1:
                self.neighbor_obs = np.ones((self.episode_length+1, self.n_rollout_threads, num_agents, max_neighbor_num, obs_shape), dtype=np.float32)*(-1)
                self.neighbor_mask = np.zeros((self.episode_length+1, self.n_rollout_threads, num_agents, max_neighbor_num), dtype=np.float32)
                self.neighbor_type_matrix = np.zeros((self.episode_length+1, self.n_rollout_threads, num_agents, max_neighbor_num), dtype=np.float32)
                self.neighbor_relation = np.zeros((self.episode_length+1, self.n_rollout_threads, num_agents, max_neighbor_num, 2), dtype=np.float32)
                self.neighbor_distances_matrix = np.zeros((self.episode_length+1, self.n_rollout_threads, num_agents, max_neighbor_num, 2), dtype=np.float32)

        self.one_hot_states = np.tile(np.eye(num_agents), (self.n_rollout_threads, 1))  
        # self.index_one_hot_states = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents, num_agents), dtype=np.float32)
        # for idx in range(self.episode_length + 1):
        #     self.index_one_hot_states[idx, :] = self.one_hot_states.copy()
        ...
        self.available_actions = np.ones((self.episode_length + 1, self.n_rollout_threads, num_agents, act_space), dtype=np.float32)
    ...
    def cal_neighbor_obs(self, obs, step, update=False):
        obs = np.array(obs)
        if self.neighbor_idxs is not None:
            for i, idxs in enumerate(self.neighbor_idxs):
                if idxs == []:
                    continue
                self.neighbor_obs[step, :, i, :len(idxs), :] = np.array(obs[:, idxs, :]).copy()
                if update:
                    self.neighbor_mask[step, :, i, :] = self.neighbor_masks[i, :]
                    self.neighbor_type_matrix[step, :, i, :] = self.neighbor_type[i, :]
                    self.neighbor_relation[step, :, i, :, :] = self.phase_relation[i, :, :]
                    self.neighbor_distances_matrix[step, :, i, :, :] = self.neighbor_distances[i, :]
                    if step == self.episode_length:
                        self.update = False

    def insert(self, share_obs, obs, rnn_states, rnn_states_critic, actions, action_log_probs,
               value_preds, rewards, masks, bad_masks=None, active_masks=None, available_actions=None, base_idx=None, update=False):

        step = self.step + base_idx
        if self._mixed_obs:
            for key in self.share_obs.keys():
                self.share_obs[key][step + 1] = share_obs[key].copy()
            for key in self.obs.keys():
                self.obs[key][step + 1] = obs[key].copy()
        else:
            self.share_obs[step + 1] = share_obs.copy()
            self.obs[step + 1] = obs.copy()

        if self.agg:
            self.cal_neighbor_obs(obs, step + 1, self.update)

        self.rnn_states[step + 1] = rnn_states.copy()
        self.rnn_states_critic[step + 1] = rnn_states_critic.copy()
        self.actions[step] = actions.copy()
        self.action_log_probs[step] = action_log_probs.copy()
        self.value_preds[step] = value_preds.copy()
        self.rewards[step] = rewards.copy()
        self.masks[step + 1] = masks.copy()
        if bad_masks is not None:
            self.bad_masks[step + 1] = bad_masks.copy()
        if active_masks is not None:
            self.active_masks[step + 1] = active_masks.copy()
        if available_actions is not None:
            self.available_actions[step + 1] = available_actions.copy()

        if update:
            self.step = (self.step + 1) % (self.episode_length//self.parallel_num)
    ...
    #     if self.agg:
    #         for i, idxs in enumerate(self.neighbor_idxs):
    #             self.neighbor_obs[self.step+1, :, i, :len(idxs), :] = np.array(obs[:, idxs, :]).copy()
    #             self.neighbor_mask[self.step+1, :, i, :len(idxs)] = 1
    ...
    def compute_returns(self, next_value, value_normalizer=None):
        if self._use_proper_time_limits:
            if self._use_gae:
                self.value_preds[-1] = next_value.copy()
                gae = 0
                ...
                self.returns[-1] = next_value.copy()
                for step in reversed(range(self.rewards.shape[0])):
                    if self._use_popart or self._use_valuenorm:
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        if self._mixed_obs:
            ...
            for key in self.index_one_hot_states.keys():
                index_one_hot_states[key] = self.index_one_hot_states[key][:-1].reshape(-1, *self.index_one_hot_states[key].shape[3:])
        else:
            share_obs = self.share_obs[:-1].reshape(-1, *self.share_obs.shape[3:])
            obs = self.obs[:-1].reshape(-1, *self.obs.shape[3:])
            # index_one_hot_states = self.index_one_hot_states[:-1].reshape(-1, *self.index_one_hot_states.shape[3:])
        ...
        if self.agg == 1:
            neighbor_obs = self.neighbor_obs[:-1].reshape(-1, *self.neighbor_obs.shape[3:])
            neighbor_mask = self.neighbor_mask[:-1].reshape(-1, *self.neighbor_mask.shape[3:])
            neighbor_types = self.neighbor_type_matrix[:-1].reshape(-1, *self.neighbor_type_matrix.shape[3:])
            neighbor_relations = self.neighbor_relation[:-1].reshape(-1, *self.neighbor_relation.shape[3:])
            neighbor_distances = self.neighbor_distances_matrix[:-1].reshape(-1, *self.neighbor_distances_matrix.shape[3:])
        ...
        for indices in sampler:
            ...
            if self._mixed_obs:
                ...
                # index_one_hot_states_batch = index_one_hot_states[indices]         
            ...
            if self.agg == 1:
                ...
                neighbor_mask_batch = neighbor_mask[indices]
                neighbor_types_batch = neighbor_types[indices]
                neighbor_relations_batch = neighbor_relations[indices]
                neighbor_distances_batch = neighbor_distances[indices]
            else:
                neighbor_obs_batch = None
                neighbor_mask_batch = None
                neighbor_types_batch = None
                neighbor_relations_batch = None
                neighbor_distances_batch = None
            yield share_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch, value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, adv_targ, available_actions_batch, neighbor_obs_batch, neighbor_mask_batch, neighbor_types_batch, neighbor_relations_batch, neighbor_distances_batch
