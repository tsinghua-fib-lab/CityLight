...
void Simulet::Init(const Config& config_) {
  ...
  if (is_python_api) {
    if (config.tl_file.empty()) {
      Warn("Disable junction");
      config.enable_junction = false;
    }
  }
  ...
  PbTls tls;
  {
    ...
    file.close();
    if (is_python_api && config.enable_junction) {
      Info("Reading traffic lights...");
      file.open(config.tl_file, std::ios::in | std::ios::binary);
      if (!file) {
        Fatal("Cannot open file: ", config.tl_file);
      }
      if (!tls.ParseFromIstream(&file)) {
        Fatal("Failed to parse agent file.");
      }
      file.close();
    }
