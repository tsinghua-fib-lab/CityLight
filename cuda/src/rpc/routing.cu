    ...
    if (in_cache) {
      ...
      if (pb_res.journeys_size() == 0) {
        ...
        if (req.type == ROUTE_TYPE_DRIVING) {
          ...
          v->start_lane = S->road.road_map[rids[0]]->right_driving_lane;
          uint n = rids.size();
          r.New(S->mem, n);
          for (int i = 0; i < n; ++i) {
            r[i] = rids[i];
          }
          v->end_lane = S->road.road_map[rids[n - 1]]->right_driving_lane;
          auto& d = v->distance_to_end;
          d.New(S->mem, 2 * n - 1);
          for (int i = n - 2; i >= 0; --i) {
            uint nr = rids[i + 1];
            bool check = false;
            for (auto* l : S->road.road_map[rids[i]]->lanes) {
          ...
          double s;
          if (req.end.is_aoi) {
            Aoi* a = v->end_aoi = S->aoi.aoi_map[req.end.id];
            bool check = false;
          ...
          for (auto& pb : pb) {
            auto& s = r[index++];
            s.lane = S->lane.lane_map[pb.lane_id()];
            s.dir = pb.moving_direction();
          }
          bool check = false;
          auto* l = r.back().lane;
          auto* a = p->end_aoi =
              S->aoi.aoi_map[pb_req.end().aoi_position().aoi_id()];
