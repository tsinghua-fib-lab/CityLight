...
void ProcessPredefinedRoute(Simulet* S, Person& p, const PbPosition& home,
  ...
  if (end.has_aoi_position()) {
    Aoi* a = v->end_aoi = S->aoi.aoi_map[end.aoi_position().aoi_id()];
    ...
    v->end_lane = S->lane.lane_map[end.lane_position().lane_id()];
  ...
  if (home.has_aoi_position()) {
    auto* a = S->aoi.aoi_map[home.aoi_position().aoi_id()];
    auto road_id = rids[0];
    auto flag = false;
    for (auto& g : a->driving_gates) {
      if (g.lane->parent_id == road_id) {
    ...
    p.runtime.lane = S->lane.lane_map[home.lane_position().lane_id()];
