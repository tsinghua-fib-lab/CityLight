...
// 限速
__device__ float PolicyToLimit(Person& p, float step_interval) {
  ...
  return IDMCarFollowAcc(p, max_speed, 1e999, 1e999, 0);
}
...
__device__ float PolicyCarFollow(Person& p, float step_interval) {
  return _CarFollow(p, p.node, p.snapshot.lane, step_interval);
}
__device__ float PolicyShadowCarFollow(Person& p, float step_interval) {
  return _CarFollow(p, p.shadow_node, p.snapshot.shadow_lane, step_interval);
}
...
__device__ float LaneAhead(Person& p, float step_interval, float distance,
  ...
  // 信控
  if (!stop && p.next_lane->parent_junction) {
    stop = p.next_lane->veh_cnt > junction_blocking_count;
    if (!stop) switch (p.next_lane->light_state) {
...
__device__ float PolicyLaneAhead(Person& p, float step_interval,
                                 uint junction_blocking_count) {
  return LaneAhead(p, step_interval, p.snapshot.lane->length - p.snapshot.s,
                   junction_blocking_count);
}
__device__ float PolicyShadowLaneAhead(Person& p, float step_interval,
                                       uint junction_blocking_count) {
  return LaneAhead(p, step_interval,
                   p.snapshot.shadow_lane->length - p.snapshot.s,
                   junction_blocking_count);
}

// 终点前减速
__device__ float PolicyToEnd(Person& p, float step_interval) {
  return IDMCarFollowAcc(p, 1e999, 0, p.snapshot.distance_to_end, 0);
}
...
// 路由强制变道
__device__ float PlanNecessaryLaneChange(Person& p, LaneChangeEnv& env,
                                         float step_interval) {
...
__device__ void UpdateAction(Person& p, float step_interval,
  ...
  // 加速度决策
  acc = PolicyToLimit(p, step_interval);
  if (acc < p.runtime.acc) {
    p.runtime.acc = acc;
    p._reason = AccReason::TO_LIMIT;
  }
  acc = PolicyToEnd(p, step_interval);
  if (acc < p.runtime.acc) {
    p.runtime.acc = acc;
    p._reason = AccReason::TO_END;
  }
  // 变道时只考虑目标车道的情况
  p.ahead_id = (uint)-1;
  if (p.snapshot.is_lane_changing) {
    acc = PolicyShadowCarFollow(p, step_interval);
    if (acc < p.runtime.acc) {
      p.runtime.acc = acc;
      p._reason = AccReason::CAR_FOLLOW_SHADOW;
    }
    acc = PolicyShadowLaneAhead(p, step_interval, junction_blocking_count);
    if (acc < p.runtime.acc) {
      p.runtime.acc = acc;
      p._reason = AccReason::LANE_AHEAD_SHADOW;
    }
  } else {
    acc = PolicyCarFollow(p, step_interval);
    if (acc < p.runtime.acc) {
      p.runtime.acc = acc;
      p._reason = AccReason::CAR_FOLLOW;
    }
    acc = PolicyLaneAhead(p, step_interval, junction_blocking_count);
    if (acc < p.runtime.acc) {
      p.runtime.acc = acc;
      p._reason = AccReason::LANE_AHEAD;
    }
  }
  // 变道决策
  if (!p.snapshot.is_lane_changing && p.snapshot.lane->parent_is_road) {
    ...
    if (p.route_lc_offset) {
      acc = PlanNecessaryLaneChange(p, env, step_interval);
      if (acc < p.runtime.acc) {
        p.runtime.acc = acc;
        p._reason = AccReason::LANE_CHANGE_N;
      }
