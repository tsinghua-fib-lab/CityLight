...
void Data::Init(Simulet* S, const PbMap& map, const PbTls& tls) {
  this->S = S;
  stream = NewStream();
  ...
  std::unordered_map<uint32_t, const PbTl*> tl_map;
  for (auto& i : tls.traffic_lights()) {
    tl_map[i.junction_id()] = &i;
  }
  for (auto& pb : map.junctions()) {
    ...
    j.lanes.New(S->mem, pb.lane_ids_size());
    {
      ...
      for (auto& pb : pb.lane_ids()) {
        auto* l = j.lanes[index++] = S->lane.lane_map[pb];
        l->parent_junction = &j;
        if (l->type == LaneType::LANE_TYPE_DRIVING) {
    ...
    if (S->config.enable_junction) {
      ...
      if (S->is_python_api) {
        ...
        if (tl_map.find(j.id) == tl_map.end()) {
          continue;
        }
        auto& tl = *tl_map[j.id];
        if (tl.phases_size()) {
