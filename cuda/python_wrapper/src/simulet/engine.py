...
class Engine:
    ...
    def __init__(
        ...
        if not hasattr(_thread_local, 'device'):
            ...
            raise RuntimeError("Cannot create multiple Engines on different device! Use simulet.parallel.ParallelEngine instead.")
        self._e = _simulet.Engine(
            map_file, tl_file, agent_file, start_step, total_step, step_interval, seed, verbose_level.value,
            agent_limit, disable_aoi_out_control, disable_junction, junction_blocking_count, junction_yellow_time,
            phase_pressure_coeff, lane_change.value, mobil_lc_forbidden_distance, lane_veh_add_buffer_size,
            lane_veh_remove_buffer_size, device,
        )
    ...
    @staticmethod
    def convert_map_agent(in_map: str, in_agent: str, out_map: str, out_tl: str, out_agent: str, out_id: str, max_agent_start_time=1e999):
        _map, _tls, _agents, ids = convert_from_cityflow(
    ...
    @staticmethod
    def from_cityflow(cityflow_config: dict, output_path=None, max_agent_start_time=1e999):
        ...
        Engine.convert_map_agent(
            ...
            out_tl=output_path + '/_simulet_tl.bin',
        ...
        return Engine(
            ...
            tl_file=output_path + '/_simulet_tl.bin',
    ...
    def get_traffic_lights(self):
        """
        Get the TrafficLights object
        """
        return TrafficLights(self.tl_file)
    ...
    def get_lane_geoms(self) -> List[NDArray[np.float32]]:
        ...
        """
        return [np.array(i, np.float32).reshape(-1, 2) for i in self._e.get_lane_geoms()]
    ...
    def get_lane_waiting_vehicle_counts(self, speed_threshold: float = 0.1) -> Union[Dict[str, int], List[int]]:
        """
    ...
    def get_lane_waiting_at_end_vehicle_counts(self, speed_threshold: float = 0.1, distance_to_end: float = 100) -> Union[Dict[str, int], List[int]]:
        """
        ...
        if self.has_id:
            return {i: j for i, j in zip(
                self.id2lane,
                self._e.get_lane_waiting_at_end_vehicle_counts(speed_threshold, distance_to_end),
            )}
        return self._e.get_lane_waiting_at_end_vehicle_counts(speed_threshold, distance_to_end)
    ...
    def get_road_lane_plans(self, road_id: int) -> List[List[slice]]:
        ...
        ```
        """
        return [[slice(a, b) for a, b in i] for i in self._e.get_road_lane_plans(road_id)]
    ...
    def get_vehicle_total_distances(self) -> Union[Dict[str, float], NDArray[np.float32]]:
        """
        Get the total traveled distance of each vehicle
        """
        if self.has_id:
            return {i: j for i, j in zip(self.id2agent, self._e.get_vehicle_total_distances())}
        return self._e.get_vehicle_total_distances()

    def get_vehicle_positions(self) -> NDArray[np.float64]:
        """
        Get the (x,y,dir) of each vehicle
        """
    ...
    def get_road_waiting_vehicle_counts(self, speed_threshold: float = 0.1) -> Union[Dict[str, int], List[int]]:
        """
    ...
    def set_vehicle_enable_batch(self, vehicle_ids: List[int], enable: Union[bool, List[bool]]):
        """
        Enable or disable vehicle `vehicle_id`
        """
        self._e.set_vehicle_enable_batch(vehicle_ids, [enable]*len(vehicle_ids) if isinstance(enable, bool) else enable)
    ...
    def set_tl_duration(self, junction_id: int, duration: int):
        ...
        NOTE: Set duration to `0` to use the predefined duration in the `tl_file`
        """
