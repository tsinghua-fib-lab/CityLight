...
if True:  # 排序保护
    from . import wolong
    sys.modules['wolong'] = wolong
    from wolong.agent.v2 import agent_pb2 as PbAgent
    from wolong.map.v2 import map_pb2 as PbMap
    from wolong.routing.v2 import routing_pb2 as PbRouting
    from wolong.traffic_light.v2 import traffic_light_pb2 as PbTl
    from wolong.trip.v2 import trip_pb2 as PbTrip
...
def convert_from_cityflow(data_map, data_agents, max_agent_start_time):
    ...
    _tls = PbTl.TrafficLights()
    dict2pb({'traffic_lights': tls}, _tls)
    _agents = PbAgent.Agents()
    dict2pb(
        {'agents': agents},
        _agents
    )
    return _map, _tls, _agents, ids
